...
import { getUserBySession, upsertGroupRide, findGroupRide, setRideRsvp, getRollCall, type RsvpStatus } from "@/lib/db";
 ...
 *        → { data: { saved, counts: {yes,maybe,no}, names?: {yes,maybe,no}, mine } }
 *        Names (first names) only for signed-in riders; everyone sees counts.
...
async function rollCall(rideId: string | null, viewerId: string | null) {
  const counts = { yes: 0, maybe: 0, no: 0 };
  const names: Record<RsvpStatus, string[]> = { yes: [], maybe: [], no: [] };
  let mine: RsvpStatus | null = null;
  if (rideId) {
    for (const r of await getRollCall(rideId)) {
      counts[r.status]++;
      names[r.status].push((r.name ?? "A rider").trim().split(/\s+/)[0] || "A rider");
      if (r.user_id === viewerId) mine = r.status;
    }
  }
  return { saved: !!rideId, counts, ...(viewerId ? { names } : {}), mine };
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ data: await rollCall(ride?.id ?? null, user?.id ?? null) });
  } catch (err) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ data: await rollCall(ride.id, user.id) });
  } catch (err) {
    return handleApiError(err);
  }
}
