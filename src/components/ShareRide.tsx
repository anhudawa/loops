...
export default function ShareRide({ route, ride }: ShareRideProps) {
  ...
  const { user } = useAuth();
  // Which ride (time|meet) was last saved to "My rides" from this sheet.
  ...
  // Sharing a ride saves it to the rider's rides ("My rides") when signed
  // in (signed out, the sheet says how to keep it). Fire and forget.
  const rideKey = `${startTime}|${cleanMeet(meetingPoint) ?? ""}`;
  const saveRide = () => {
    if (!when || !user) return;
    const key = rideKey;
    fetch("/api/rides", {
      ...
      keepalive: true,
    }).then((r) => { if (r.ok) setSavedKey(key); }).catch(() => {});
  };
    ...
    <>
      ...
      {open && typeof document !== "undefined" && createPortal(
        <div className="fixed inset-0 z-[1000] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm" onClick={closeSheet}>
          ...
          >
            {/* Header: neutral — the sheet offers WhatsApp, Instagram and the rest */}
            <div className="px-6 pt-5 pb-4" style={{ background: "var(--accent-glow)" }}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-10 h-10 shrink-0 rounded-full flex items-center justify-center" style={{ background: "var(--accent)", color: "#0a0a0a" }}>
                    {PEOPLE_ICON}
                  </div>
                  <div className="min-w-0">
                    <h3 id="share-ride-title" className="text-lg font-extrabold tracking-tight" style={{ color: "var(--text)" }}>{forwarding ? "Forward this ride" : "Invite friends to ride"}</h3>
                    <p className="text-xs truncate" style={{ color: "var(--text-muted)" }}>{route.name} · {route.region || route.county}</p>
                  </div>
                </div>
            ...
            <div className="px-6 py-5">
              ...
              {/* Where the ride lives afterwards */}
              {user ? (
                savedKey === rideKey && (
                  <p className="text-xs mt-3 text-center" style={{ color: "var(--text-secondary)" }} role="status" data-testid="ride-saved">
                    Saved to <a href="/rides" className="font-bold underline underline-offset-2" style={{ color: "var(--accent)" }}>My rides</a> — see who&apos;s in there.
                  </p>
                )
              ) : (
                <p className="text-xs mt-3 text-center" style={{ color: "var(--text-muted)" }}>
                  <a href={`/login?redirect=${encodeURIComponent(`/ride/${route.id}?${new URLSearchParams({ t: startTime, ...(cleanMeet(meetingPoint) ? { m: cleanMeet(meetingPoint)! } : {}) }).toString()}`)}`} className="font-bold underline underline-offset-2" style={{ color: "var(--text-secondary)" }}>Log in</a>{" "}
                  to keep this ride in My rides and see who&apos;s coming.
                </p>
              )}
            </div>
          </div>
